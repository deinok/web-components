...
class UserCard extends HTMLElement{
    ...
    public constructor(){
        super();
        this.addEventListener("click",(eventMap:HTMLElementEventMap["click"]):void =>{
            this.toggleCard();
        });
    }

    public toggleCard():void {
        const element:HTMLElement = this.shadowRoot.querySelector('.card__hidden-content');
        const btn:HTMLButtonElement = this.shadowRoot.querySelector('.card__details-btn');
        btn.innerHTML = element.style.display === 'none' ? 'Less Details' : 'More Details';
        element.style.display = element.style.display === 'none' ? 'block' : 'none';
    }

    public render(userData:any) {
    ...
    private connectedCallback():void {
        ...
        const template:HTMLTemplateElement = currentDocument.querySelector('#user-card-template');
